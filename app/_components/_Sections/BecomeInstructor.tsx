...
function BecomeInstructor({
	...
	return (
		<div key={heading} className='flex '>
			<section className='container flex px-4 py-6 mx-auto md:px-6 lg:py-24  justify-between [@media(max-width:1300px)]:flex-col [@media(max-width:1300px)]:gap-[20px]  '>
				<div
					style={{
						background: "linear-gradient(90deg, #CC522B 0%, #FF6636 100%)",
					}}
					className='flex flex-col md:flex-row overflow-hidden relative justify-between text-white p-[40px]  w-[648px]   md:min-h-[270px]  [@media(max-width:1300px)]:self-baseline  '
				>
					<div className='max-w-lg gap-[24px] flex flex-col text-center md:text-left'>
						...
						<div className='bg-white self-baseline text-[#FF6636] gap-[12px] cursor-pointer flex items-center  px-[24px] py-[10px] text-[16px] font-[600]'>
							<Link href={String(link?.url)}>{link?.text}</Link>
				...
				<div className='flex flex-col gap-[27px] p-[40px] [@media(max-width:1300px)]:self-end '>
					<h1 className='text-[32px] leading-[40px] font-[600] w-[568px] '>
						{stepsHeading}
					</h1>
